...
for c in c_array:
    temp_error = []
    kf = KFold(n_splits=5)
    for train, test_we in kf.split(X_wd):
        model_we = Ridge(fit_intercept=False, alpha=(1 / (2 * c))).fit(XX_wd[train], yy_wd[train])
        y_pred_we = model_we.predict(XX_wd[test_we])
        temp_error.append(mean_squared_error(yy_wd[test_we], y_pred_we))
    mean_error.append(np.mean(temp_error))
    std_error.append(np.std(temp_error))

print("Mean error list is ", mean_error)
plot.errorbar(c_array, mean_error, std_error)
plot.xlabel("C")
plot.ylabel("Mean Square Error")
plot.show()

XX_wd = PolynomialFeatures(2).fit_transform(XX_wd)
train, test_wd = train_test_split(np.arange(0, yy_wd.size), test_size=0.2)
model_wd = Ridge(fit_intercept=False, alpha=(1 / (2 * 0.001))).fit(XX_wd[train], yy_wd[train])
# model = KNeighborsRegressor(n_neighbors=5, weights='uniform').fit(XX[train], yy[train])
print("Model Intercepts and coefficients are", model_wd.intercept_, model_we.coef_)
...
XX_we = PolynomialFeatures(1).fit_transform(XX_we)
train, test_we = train_test_split(np.arange(0, yy_we.size), test_size=0.2)
model_we = Ridge(fit_intercept=False, alpha=(1 / (2 * 0.001))).fit(XX_we[train], yy_we[train])
# model = KNeighborsRegressor(n_neighbors=5, weights='uniform').fit(XX[train], yy[train])
print("Model Intercepts and coefficients are", model_we.intercept_, model_we.coef_)
