...
weekly_len = math.floor(7 * 24 * 60 * 60 / time_diff)
...
for i in range(lag - 1, -1, -1):
    print(i)
    temp = y[lag_weekly * weekly_len - i: int(lag_weekly * weekly_len - i + length)]
    X = np.column_stack((X, temp))
...
print("=========================================================================")
print(r2_score(y_true=yy[test], y_pred=XX[test][:, 4]))
print(mean_squared_error(y_true=yy[test], y_pred=XX[test][:, 4]))